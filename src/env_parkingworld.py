...
class ParkingWorld:
    ...
    def state_reward(self, state):
        # get reward to being in a given state
    ...
    def get_reward(self, last_state, state):
        # get the reward from transitioning from one state to the next
    ...
    def get_transition(self, last_state, last_action):
        # get possible transition rewards and probabilities
        transition = {}
        for state in self.states:
            transition[state] = {}
            transition[state]['reward'] = self.get_reward(last_state, state)
            transition[state]['probability'] = self.get_state_probability(state, last_state, last_action)
        return transition
    ...
    def env_step(self, last_state, last_action):
        ...
        reward = self.get_reward(last_state, new_state)
        terminal = False
